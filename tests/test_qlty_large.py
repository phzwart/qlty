...
@pytest.mark.parametrize("step, border", [
    ((16, 32), None),
    ((16, 32), (0, 0)),
    ((8, 8), (2, 3)),
])
def test_LargeNCYXQuilt(temp_dir, step, border):
    ...
    ain, aout = quilt.unstitch_and_clean_sparse_data_pair(imgs_in, labels, missing_label)
    ...
    # Clean up
    for suffix in ["_mean_cache.zarr", "_std_cache.zarr", "_norma_cache.zarr", "_mean.zarr"]:
...
def test_LargeNCZYXQuilt(temp_dir, step, border):
    ...
    for ii in range(2):
        img = []
        for jj in range(2):
            tmp = (np.sin((jj + 1) * X + ii * np.pi / 3.0) + 
...
def test_LargeNCYXQuilt_with_std(temp_dir):
    ...
    # Clean up
    for suffix in ["_mean_cache.zarr", "_std_cache.zarr", "_norma_cache.zarr", "_mean.zarr", "_std.zarr"]:
        path = filename + suffix
        if os.path.exists(path):
            shutil.rmtree(path, ignore_errors=True)
